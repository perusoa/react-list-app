...
export default function ListItem({ listKey, listItemId, listText, onListItemDelete, onListItemMove }) {
  return (
    <div className="flex items-center bg-gray-100 rounded-md py-2 px-3 w-100 mb-3">
      <p className="font-medium mr-auto pr-2">{listText}</p>
      <div onClick={() => onListItemDelete(listKey, listItemId)} className="p-1 bg-red-300 rounded-md pointer mr-2 cursor-pointer">
        <XIcon className="w-6 text-red-800" />
      </div>
      <div onClick={() => onListItemMove(listKey, listItemId)} className="p-1 bg-gray-300 rounded-md pointer cursor-pointer">
